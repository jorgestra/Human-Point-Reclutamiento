...
async def sync_candidate_areas(candidate_id: str, payload: dict, user: dict = Depends(check_role([UserRole.ADMIN, UserRole.RECRUITER]))):
    area_ids = payload.get('ids', []) if isinstance(payload, dict) else []
    cand = await database.fetch_one("SELECT id FROM ATS_CANDIDATOS WHERE id = ? AND tenant_id = ?", (candidate_id, user['tenant_id']))
    if not cand:
        raise HTTPException(status_code=404, detail="Candidato no encontrado")
...
async def sync_candidate_languages(candidate_id: str, payload: dict, user: dict = Depends(check_role([UserRole.ADMIN, UserRole.RECRUITER]))):
    language_ids = payload.get('ids', []) if isinstance(payload, dict) else []
    cand = await database.fetch_one("SELECT id FROM ATS_CANDIDATOS WHERE id = ? AND tenant_id = ?", (candidate_id, user['tenant_id']))
    if not cand:
        raise HTTPException(status_code=404, detail="Candidato no encontrado")
...
@api_router.get("/tenant/public-config")
async def get_tenant_public_config(tenant_id: str = "default"):
    """Endpoint público — no requiere auth. Usado en pantalla de login."""
    row = await database.fetch_one(
        "SELECT * FROM ATS_TENANTS WHERE id = ?", (tenant_id,)
    )
    if not row:
        return {"name": "Human Point", "logo_url": None, "primary_color": "#004aad"}
    d = serialize_doc(row)
    if not d.get('name') and d.get('company_name'):
        d['name'] = d['company_name']
    return {"name": d.get('name'), "logo_url": d.get('logo_url'), "primary_color": d.get('primary_color', '#004aad'), "website": d.get('website'), "short_name": d.get('short_name')}
...
async def list_hirings(page: int = 1, limit: int = 20, empresa_id: Optional[str] = None, user: dict = Depends(get_current_user)):
    ...
    where_with_alias = where.replace("WHERE tenant_id", "WHERE h.tenant_id").replace("AND empresa_id", "AND h.empresa_id")
    join_sql = f"SELECT h.*, e.name AS empresa_name FROM ATS_CONTRATACIONES h LEFT JOIN ATS_EMPRESAS e ON e.id = h.empresa_id {where_with_alias} ORDER BY h.created_at DESC"
